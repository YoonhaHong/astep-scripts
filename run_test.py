...
async def main(args):
    ...
    bitpath = args.outdir+"/run_"+str(args.name)+fname+time.strftime("%Y%m%d_%H%M%S")+".log"
#    csvpath = args.outdir+"/run_"+str(args.runnumber)+fname+time.strftime("%Y%m%d_%H%M%S")+".csv"
    logname = args.outdir+"/runlog_"+str(args.name)+fname+time.strftime("%Y%m%d_%H%M%S")+".log"
